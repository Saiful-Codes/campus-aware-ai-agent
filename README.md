...
The backend is **not a single LLM call** — it is an intent-routing orchestrator that dispatches each query to the right pipeline (live sensors, historical sensor analytics, retrieval-augmented generation, or a general conversational fallback), with guardrails that prevent the model from inventing official university information.
...
See [`docs/architecture.md`](docs/architecture.md) for the full routing, RAG, and Text-to-Flux design.
...
| [`docs/architecture.md`](docs/architecture.md) | Routing, RAG, Text-to-Flux, sensor systems, guardrails |
